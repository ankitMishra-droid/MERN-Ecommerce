...
const HomeProductCategory = () => {
  ...
  const [sortedFilter, setSortedFilter] = useState(false);
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="lg:grid grid-cols-[200px,1fr]">
        {/* Sidebar */}
        <div className="bg-slate-300 p-2 overflow-y-scroll">
          <div>
          ...
          <div>
            <h3
              className="text-base uppercase text-violet-600 font-medium pb-1 cursor-pointer"
            ...
            {mobileFiltersOpen && (
              <form className="text-sm flex flex-col gap-2 py-2 border-t border-slate-400">
        ...
        <div className="px-4">
          {loading ? (
            ...
            <>
              <p className="my-4 lg:my-0">Search Results: {sortedData.length}</p>
              {sortedData.length > 0 && (
